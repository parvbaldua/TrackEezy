        ...
        <div className={styles.container}>
            ...
            <Card className={styles.profileCard}>
                {session ? (
                    <>
                        {session.user?.image ? (
                            <img src={session.user.image} alt="Profile" className={styles.avatar} style={{ fontSize: 0 }} />
                        ) : (
                            <div className={styles.avatar}>{session.user?.name?.[0] || "U"}</div>
                        )}
                        <div className={styles.info}>
                            <h2 className={styles.name}>{session.user?.name}</h2>
                            <p className={styles.role}>{session.user?.email}</p>
                    ...
                    <div className="flex flex-col items-start gap-4 w-full">
                        ...
                        <Button onClick={() => signIn("google")} variant="primary" className="w-full sm:w-auto">
                            Sign In with Google
                        </Button>
                    </div>
                )}
                            ...
                            <ConnectionInput onConnect={async (url) => {
                                ...
                                try {
                                    const res = await fetch('/api/sync', {
                                        ...
                                        body: JSON.stringify({ sheetUrl: url, action: 'INIT' })
                                    });
                                    ...
                                    console.error(err);
                                    alert("Could not connect. Ensure you have Edit access to the sheet.");
                                }
                            }} />
                        </div>
                    )}
...
function ConnectionInput({ onConnect }: { onConnect: (url: string) => void }) {
    ...
    return (
        <div className="flex flex-col gap-3 w-full">
            ...
            <p className="text-xs text-[hsl(var(--text-muted))]">
                *Create a new sheet with columns: Name, SKU, Qty, Price
            </p>
        </div>
    );
}
